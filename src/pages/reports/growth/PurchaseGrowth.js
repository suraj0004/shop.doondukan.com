...
class PurchaseGrowth extends Component {
  ...
  renderChart() {
    axios.get(UrlService.purchaseGrowthUrl(), {
      params: {
  ...
  render() {
    ...
    const options = {
			...
			exportEnabled: true,
			title: {
				text: "Purchase Growth " +( (this.state.type === "quantity")?"By Quantity":"By Rupees")
			},
			axisY: {
				title: (this.state.type === "quantity")?"Item Bought":"Purchase in Rs.",
				includeZero: false,
			},
			data: [
			{
    ...
    var table_label = `Purchase Growth -`;
